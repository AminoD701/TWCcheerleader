  ...
  const init = () => {
    ...
    setInterval(() => {
      ...
      if (now !== lastDaily) {
        lastDaily = now;
        syncCurrentIntoHistory();
        document.querySelectorAll('[data-gacha-history-entry]').forEach(el => { el.textContent = `📚 抽卡紀錄 (${readHistory().length})`; });
      }
      addGameHubEntry();
      addFloatingEntry();
    }, 1000);
